...
test('六冲非季节力量 Guard 锁定原文支类匹配并要求独立 element-presence scope', () => {
    const guards = Object.fromEntries(assessment.assessmentGuardRegistry.rules.map((item) => [item.id, item.statement]));
    ...
    assert(text.includes('有木／无金') && text.includes('element-presence scope contract'), 'GUARD-020 未要求独立五行有无 scope');
    assert(text.includes('不得自行改换观察口径'), 'GUARD-020 未阻止随意改口径');
});

test('Element Presence Guard 固定表层干支 scope 并排除藏干／派生结构反推', () => {
    const guards = Object.fromEntries(assessment.assessmentGuardRegistry.rules.map((item) => [item.id, item.statement]));
    const text = guards['BAZI-ASSESS-GUARD-021'] || '';
    assert(text.includes('四柱表层干支'), 'GUARD-021 未固定表层干支 scope');
