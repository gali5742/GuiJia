...
const classifyVector = (vector) => {
  if (!routeHead || !localGateHeads || !logisticThresholds) throw new Error('请先训练 v0.8');
...
const train = async ({ onStage, onEmbeddingProgress } = {}) => {
  ...
  onStage?.('done', '训练完成：v0.8 targeted 22-route boundary refinement');
  return {
...
const classify = async (text) => {
  if (!routeHead || !localGateHeads) throw new Error('请先训练 v0.8');
...
export const semanticRouterPocV08 = {
  modelId:MODEL_ID,
