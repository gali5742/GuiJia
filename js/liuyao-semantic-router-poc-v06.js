...
const DEV_BENCHMARK_URL = new URL('../data/liuyao-semantic-route-eval-v0.1.json', import.meta.url);
...
const ensureData = async () => {
  if (!trainingData) {
    const [base, augmentation, targeted] = await Promise.all([
      fetchJson(TRAIN_BASE_URL),
      fetchJson(TRAIN_AUG_URL),
      fetchJson(TRAIN_TARGETED_URL)
    ]);
    trainingData = mergeTraining(base, augmentation, targeted);
  }
  if (!benchmarkData) benchmarkData = await fetchJson(DEV_BENCHMARK_URL);
  routeIds = Object.keys(trainingData.routes || {});
  return { trainingData, benchmarkData };
};
...
const classifyVector = (vector) => {
  if (!routeHead || !localGateHeads || !logisticThresholds) throw new Error('请先训练 v0.6');
  ...
  return {
    ...
    routeMargin,
    scores,
    gate:{ score:gateScore, threshold, accepted, predicted:accepted ? top1.id : '__other__' }
  };
};
...
const train = async ({ onStage, onEmbeddingProgress } = {}) => {
  ...
  onStage?.('calibration', '正在用独立 validation 分 route 校准 acceptance threshold…');
  ...
  onStage?.('done', '训练完成：v0.5 架构 + targeted data refinement v0.6');
  return {
...
const classify = async (text) => {
  if (!routeHead || !localGateHeads) throw new Error('请先训练 v0.6');
  const [vector] = await embedTexts([text]);
  return classifyVector(vector);
};

const runDevelopmentBenchmark = async ({ onProgress } = {}) => {
  await ensureData();
  if (!routeHead || !localGateHeads) throw new Error('请先训练 v0.6');
  const rows = [];
  for (const [label, texts] of Object.entries(benchmarkData.samples || {})) {
    for (const text of texts) rows.push({ text, label, known:label !== '__other__', kind:label === '__other__' ? 'benchmark-other' : 'benchmark-known', targets:[] });
  }
  const vectors = [];
  const chunkSize = 20;
...
export const semanticRouterPocV06 = {
  modelId:MODEL_ID,
