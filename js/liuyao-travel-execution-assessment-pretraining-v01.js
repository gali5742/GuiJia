(function (global) {
    ...
    if (!assessmentApi?.validateAssessmentEnvelope) {
        throw new Error('liuyao-domain-assessment-pretraining-v01.js must be loaded before liuyao-travel-execution-assessment-pretraining-v01.js');
    }

    const VERSION = '0.1';
    const STATUS = 'isolated_candidate_not_registered';
    const EVALUATOR_REF = 'AE-TV-EXEC-001';
    ...
    const evaluateTravelExecution = (packet) => {
        ...
        if (validation.status !== 'valid') {
            return baseEnvelope(
                ...
                'not_assessed',
                [],
                [`${EVALUATOR_REF}:invalid_evidence_packet`],
                validation.issues
            );
        }
    ...
    const describeCandidate = () => ({
        ...
        excludedTypes:['safety_support','hazard_pressure'],
        scoringEnabled:false,
        probabilityEnabled:false
    });

    GuiJia.liuyaoTravelExecutionAssessmentPretrainingV01 = Object.freeze({
        version:VERSION,
        status:STATUS,
        evaluatorRef:EVALUATOR_REF,
        currentRuntimeReachable:false,
