(function (global) {
    ...
    const FORBIDDEN_ASSESSMENT_OUTPUTS = Object.freeze([
        'probability',
        'scalarScore',
    ...
    const validateDimensionAssessment = (assessment) => {
        ...
        if (!assessment || typeof assessment !== 'object' || Array.isArray(assessment)) {
            return { status:'invalid', issues:[issue('assessment_object_required')] };
        }
        ...
        if (!Array.isArray(assessment.evidenceRefs)) {
            issues.push(issue('evidence_refs_array_required'));
        } else if (assessment.evidenceRefs.some((ref) => !hasText(ref))) {
            issues.push(issue('evidence_ref_invalid'));
        }

        FORBIDDEN_ASSESSMENT_OUTPUTS.forEach((key) => {
            if (Object.prototype.hasOwnProperty.call(assessment, key)) {
                issues.push(issue('forbidden_comparator_input_output_field', { field:key }));
            }
        });
    ...
    const resolutionGate = (left, right) => {
        ...
        if (statuses.includes('unresolved')) {
            return { comparisonStatus:'unresolved', reason:'resolution_unresolved' };
        }
        if (statuses.includes('partial')) {
            return { comparisonStatus:'partial', reason:'resolution_partial' };
        }
        if (statuses.includes('not_applicable')) {
            return { comparisonStatus:'incomparable', reason:'dimension_not_applicable' };
        }
        return null;
    };
    ...
    const compareDimensionAssessments = (left, right) => {
        ...
        if (left.semanticMeaning !== right.semanticMeaning) {
            result.reason = 'semantic_meaning_mismatch';
            return result;
        }
        if (left.contractFamily !== right.contractFamily) {
            result.reason = 'contract_family_mismatch';
            return result;
        }
        ...
        result.reason = ACTIVE_COMPARATORS.length
            ? 'compatible_comparator_not_selected'
    ...
    GuiJia.liuyaoDomainComparatorPretrainingV01 = Object.freeze({
        version:VERSION,
        status:STATUS,
        currentRuntimeReachable:false,
        RESOLUTION_STATUSES,
