...
const normalizeVector = (vector) => {
  let norm2 = 0;
  for (let i = 0; i < vector.length; i += 1) norm2 += vector[i] * vector[i];
...
const flattenPositives = (split) => {
  ...
  for (const routeId of routeIds) {
    for (const text of trainingData.routes[routeId][split] || []) {
      rows.push({ text, label:routeId, known:true, kind:'route', targets:[] });
    }
  }
  return rows;
};
...
const buildCentroid = (vectors) => {
  const mean = new Float32Array(VECTOR_SIZE);
  for (const vector of vectors) {
    for (let j = 0; j < VECTOR_SIZE; j += 1) mean[j] += vector[j];
  }
  const scale = 1 / Math.max(1, vectors.length);
  for (let j = 0; j < VECTOR_SIZE; j += 1) mean[j] *= scale;
...
const centroidScore = (routeId, vector) => {
  const centroid = centroids?.[routeId];
  if (!centroid) throw new Error(`缺少 centroid: ${routeId}`);
  return dot(centroid, vector);
};
...
const classifyVector = (vector) => {
  if (!routeHead || !localGateHeads || !centroids || !logisticThresholds || !centroidThresholds) throw new Error('请先训练 v0.5');
  const scores = routeScores(vector);
  const top1 = scores[0];
  const top2 = scores[1];
  const routeMargin = top1.score - top2.score;
  const aScore = logisticScore(top1.id, vector);
  const bScore = centroidScore(top1.id, vector);
  const aThreshold = logisticThresholds[top1.id].threshold;
  const bThreshold = centroidThresholds[top1.id].threshold;
  const aAccepted = aScore >= aThreshold;
  const bAccepted = bScore >= bThreshold;
  return {
    top1,
    top2,
    routeMargin,
    scores,
    logistic:{ score:aScore, threshold:aThreshold, accepted:aAccepted, predicted:aAccepted ? top1.id : '__other__' },
    centroid:{ score:bScore, threshold:bThreshold, accepted:bAccepted, predicted:bAccepted ? top1.id : '__other__' }
  };
};
...
const evaluateRows = (rows, vectors) => rows.map((row, index) => {
  ...
  return {
    ...row,
    ...result,
    logisticCorrect:result.logistic.predicted === row.label,
    centroidCorrect:result.centroid.predicted === row.label
  };
});

const summarizeMethod = (results, method) => {
  const known = results.filter((row) => row.label !== '__other__');
  const other = results.filter((row) => row.label === '__other__');
  const acceptedKnown = known.filter((row) => row[method].accepted);
  const correctAcceptedKnown = acceptedKnown.filter((row) => row[method].predicted === row.label);
  const rejectedOther = other.filter((row) => !row[method].accepted);
  const exact = results.filter((row) => row[`${method}Correct`]);
  return {
...
const summarize = (results) => {
  ...
  return {
    ...
    logistic:summarizeMethod(results, 'logistic'),
    centroid:summarizeMethod(results, 'centroid')
  };
};
...
const train = async ({ onStage, onEmbeddingProgress } = {}) => {
  ...
  onStage?.('logistic', `正在训练 ${routeIds.length} 个 route-conditioned Logistic Gate…`);
  localGateHeads = {};
  ...
  onStage?.('centroid', '正在建立各 route 的训练集 centroid…');
  centroids = {};
  for (const routeId of routeIds) {
    const routeVectors = [];
    for (let i = 0; i < trainPositives.length; i += 1) if (trainPositives[i].label === routeId) routeVectors.push(trainPositiveVectors[i]);
    centroids[routeId] = buildCentroid(routeVectors);
  }

  onStage?.('calibration', '正在用独立 validation 分 route 校准 A/B acceptance threshold…');
  ...
  for (const routeId of routeIds) {
    ...
    for (const item of localRows) {
      const a = logisticScore(routeId, item.vector);
      const b = centroidScore(routeId, item.vector);
      (item.positive ? aPos : aNeg).push(a);
      (item.positive ? bPos : bNeg).push(b);
    }
    logisticThresholds[routeId] = chooseThreshold(aPos, aNeg, { defaultThreshold:0.5 });
    centroidThresholds[routeId] = chooseThreshold(bPos, bNeg, { defaultThreshold:0.6 });
  }

  const validationResults = evaluateRows(validationRows, validationVectors);
  const validationMetrics = summarize(validationResults);
  onStage?.('done', '训练完成：Route classifier + A 局部 Logistic Gate + B 类别 centroid threshold');
  return {
...
const classify = async (text) => {
  if (!routeHead || !localGateHeads || !centroids) throw new Error('请先训练 v0.5');
  const [vector] = await embedTexts([text]);
  return classifyVector(vector);
};

const runEvaluation = async ({ onProgress } = {}) => {
  await ensureData();
  if (!routeHead || !localGateHeads || !centroids) throw new Error('请先训练 v0.5');
  const rows = [];
  for (const [label, texts] of Object.entries(evalData.samples || {})) {
    for (const text of texts) rows.push({ text, label, known:label !== '__other__', kind:label === '__other__' ? 'eval-other' : 'eval-known', targets:[] });
  }
  const vectors = [];
  const chunkSize = 20;
  ...
  const results = evaluateRows(rows, vectors);
  return { metrics:summarize(results), results };
};

export const semanticRouterPocV05 = {
  modelId:MODEL_ID,
