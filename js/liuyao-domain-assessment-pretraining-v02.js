(function (global) {
    ...
    const validateAssessmentEnvelope = (assessment) => {
        ...
        ['readingRef','assessmentRef','assessmentVersion','contractFamily','eventType','duty','dimensionId','semanticMeaning'].forEach((key) => {
    ...
    const validateEvaluatorRequest = (request) => {
        ...
        ['readingRef','eventType','duty','dimensionId','semanticMeaning','contractFamily','assessmentRef','assessmentVersion'].forEach((key) => {
    ...
    const bindAssessmentForComparison = (assessment, explicitAlternativeId = null) => {
        ...
        if (explicitAlternativeId !== null && !providedAlternativeId) return { status:'invalid', input:null, issues:[issue('comparison_alternative_id_invalid')] };
        if (envelopeAlternativeId && providedAlternativeId && envelopeAlternativeId !== providedAlternativeId) {
            return { status:'invalid', input:null, issues:[issue('comparison_alternative_id_mismatch',{envelopeAlternativeId,providedAlternativeId})] };
        }
        const alternativeId = providedAlternativeId || envelopeAlternativeId;
        if (!alternativeId) return { status:'unbound', input:null, issues:[issue('comparison_alternative_id_required')] };
        return {
            status:'bound',
            input:{
                readingRef:assessment.readingRef,
                alternativeId,
    ...
    const findRegisteredEvaluator = (request) => ACTIVE_EVALUATORS.find((entry) => evaluatorKey(entry) === evaluatorKey(request)) || null;

    const unresolvedEnvelope = (request,reason,extraIssues=[]) => ({
        readingRef:hasText(request?.readingRef) ? request.readingRef : 'unresolved-reading',
    ...
    const buildAssessmentReadiness = () => ({
        ...
        readingRefCarriedToComparator:true,
    ...
    GuiJia.liuyaoDomainAssessmentPretrainingV02 = Object.freeze({
        version:VERSION,
        status:STATUS,
        currentRuntimeReachable:false,
        RESOLUTION_STATUSES,
