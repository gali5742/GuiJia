...
const TRAIN_TARGETED22_URL = new URL('../data/liuyao-semantic-route-training-v0.5-targeted-22.json', import.meta.url);
...
const mergeTraining = (base, augmentation, targeted, expansion, expansionLabelPatch, targeted22, inventory) => {
  ...
  if (targeted22?.version !== '0.5-targeted-22' || targeted22?.status !== 'development') {
    throw new Error('v0.5 targeted-22 metadata mismatch');
  }
  const routes = {};
  for (const id of ids) {
    const focused22 = targeted22.routes?.[id] || { train:[], validation:[] };
    if (base.routes?.[id]) {
      if (!augmentation.routes?.[id]) throw new Error(`augmentation 缺少旧 route: ${id}`);
      const focused = targeted.routes?.[id] || { train:[], validation:[] };
      routes[id] = {
        train:[...(base.routes[id].train || []), ...(augmentation.routes[id].train || []), ...(focused.train || []), ...(focused22.train || [])],
        validation:[...(base.routes[id].validation || []), ...(augmentation.routes[id].validation || []), ...(focused.validation || []), ...(focused22.validation || [])]
      };
      continue;
    }
    ...
    routes[id] = {
      train:[...(expanded.train || []), ...(focused22.train || [])],
      validation:[...(expanded.validation || []), ...(focused22.validation || [])]
    };
  }
  return {
    version:'0.8.1-balanced-route-head',
    routes,
    hardNegatives:{
      train:[
        ...
        ...(targeted22.hardNegatives?.train || [])
      ],
      validation:[
        ...
        ...(targeted22.hardNegatives?.validation || [])
      ]
    }
  };
};
...
const ensureData = async () => {
  if (!trainingData || !inventoryData) {
    const [base, augmentation, targeted, expansion, expansionLabelPatch, targeted22, inventory] = await Promise.all([
      ...
      fetchJson(TRAIN_TARGETED22_URL),
    ...
    trainingData = mergeTraining(base, augmentation, targeted, expansion, expansionLabelPatch, targeted22, inventory);
...
const buildBalancedClassWeights = (rows) => {
  const counts = Object.fromEntries(routeIds.map((id) => [id, 0]));
  for (const row of rows) {
    if (row.label !== '__other__' && Object.hasOwn(counts, row.label)) counts[row.label] += 1;
  }
  const total = Object.values(counts).reduce((sum, value) => sum + value, 0);
  const classCount = routeIds.length;
  const weights = {};
  for (const id of routeIds) {
    if (!counts[id]) throw new Error(`route head 缺少训练样本: ${id}`);
    weights[id] = total / (classCount * counts[id]);
  }
  return { counts, weights };
};

const trainBalancedMultinomialRouteHead = (rows, vectors, { epochs=220, learningRate=0.82, l2=0.0005 } = {}) => {
  const classCount = routeIds.length;
  ...
  const balance = buildBalancedClassWeights(rows);
  for (let epoch = 0; epoch < epochs; epoch += 1) {
    ...
    for (let i = 0; i < rows.length; i += 1) {
      ...
      const sampleWeight = balance.weights[rows[i].label];
      ...
      for (let c = 0; c < classCount; c += 1) {
        const error = (probs[c] - (c === targetIndex ? 1 : 0)) * sampleWeight;
        gradBiases[c] += error;
  ...
  return { weights, biases, classCounts:balance.counts, classWeights:balance.weights };
};
...
const classifyVector = (vector) => {
  if (!routeHead || !localGateHeads || !logisticThresholds) throw new Error('请先训练 v0.8.1');
...
const train = async ({ onStage, onEmbeddingProgress } = {}) => {
  ...
  const routeTrainIndexes = [];
  for (let i = 0; i < trainRows.length; i += 1) {
    if (trainRows[i].label !== '__other__') routeTrainIndexes.push(i);
  }
  const routeTrainRows = routeTrainIndexes.map((index) => trainRows[index]);
  const routeTrainVectors = routeTrainIndexes.map((index) => trainVectors[index]);

  onStage?.('route', `正在用 ${routeTrainRows.length} 条 known 样本训练 class-balanced 22 类 Route classifier…`);
  routeHead = trainBalancedMultinomialRouteHead(routeTrainRows, routeTrainVectors);
  await new Promise((resolve) => setTimeout(resolve, 0));

  onStage?.('logistic', `正在训练 ${routeIds.length} 个未改动的 route-conditioned Logistic Acceptance Gate…`);
  localGateHeads = {};
  ...
  onStage?.('calibration', '正在用同一 Validation 分 route 校准 acceptance threshold…');
  ...
  const routeClassCounts = routeHead.classCounts;
  const routeClassWeights = routeHead.classWeights;
  const countValues = Object.values(routeClassCounts);
  onStage?.('done', '训练完成：v0.8.1 contrastive-known + class-balanced route head');
  return {
    ...
    routeHeadTrainCount:routeTrainRows.length,
    routeHeadMinClassCount:Math.min(...countValues),
    routeHeadMaxClassCount:Math.max(...countValues),
    routeClassCounts,
    routeClassWeights,
...
const classify = async (text) => {
  if (!routeHead || !localGateHeads) throw new Error('请先训练 v0.8.1');
...
export const semanticRouterPocV081 = {
  modelId:MODEL_ID,
  modelDtype:MODEL_DTYPE,
  version:'0.8.1',
  experiment:'same-v0.8-corpus_contrastive-known-in-route-head_class-balanced-loss',
