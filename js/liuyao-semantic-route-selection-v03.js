(function (global) {
  ...
  function decide({ arbitration=null, head=null, evidence=null, routeabilityDisposition=null } = {}) {
    ...
    if (support && support.compatibility.status !== 'contradicted') {
      const otherConfirmed = evaluated.filter((candidate) => (
        candidate.routeId !== support.routeId && candidate.compatibility.status === 'confirmed'
      ));
    ...
    if (top1 && top1.compatibility.status !== 'contradicted') {
      return selected(top1, 'fallback_head_top1');
    }
    if (top2 && top2.compatibility.status !== 'contradicted') {
      return selected(top2, 'fallback_head_top2_after_top1_contradiction');
    }
    return unresolved('no_noncontradicted_head_candidate');
  }

  GuiJia.liuyaoSemanticRouteSelectionV03 = Object.freeze({
    version:VERSION,
    buildCandidateSet,
    decide
  });
