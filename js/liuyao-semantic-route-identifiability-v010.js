...
const trainLogistic = (rows, vectors, { epochs=360, learningRate=0.42, l2=0.0015 } = {}) => {
  ...
  for (let epoch = 0; epoch < epochs; epoch += 1) {
    ...
    for (let i = 0; i < rows.length; i += 1) {
      const y = rows[i].identifiable ? 1 : 0;
...
const probabilityFromVector = (vector) => {
  if (!gate) throw new Error('Route Identifiability 尚未训练');
  return sigmoid(dot(gate.weights, vector) + gate.bias);
};
const statsAt = (rows, candidateThreshold) => {
  const positives = rows.filter((row) => row.identifiable);
  const negatives = rows.filter((row) => !row.identifiable);
  const identifiableRecall = safeRatio(positives.filter((row) => row.probability >= candidateThreshold).length, positives.length);
  const unresolvedRejection = safeRatio(negatives.filter((row) => row.probability < candidateThreshold).length, negatives.length);
  return {
    threshold:candidateThreshold,
    identifiableRecall,
    unresolvedRejection,
    balancedAccuracy:(identifiableRecall + unresolvedRejection) / 2,
    minSide:Math.min(identifiableRecall, unresolvedRejection)
  };
};
const calibrateThreshold = (rows) => {
  ...
  for (const candidate of unique) {
    ...
    if (!best || current.balancedAccuracy > best.balancedAccuracy + 1e-12 ||
      ...
      (Math.abs(current.balancedAccuracy - best.balancedAccuracy) <= 1e-12 && Math.abs(current.minSide - best.minSide) <= 1e-12 && current.unresolvedRejection > best.unresolvedRejection + 1e-12)) {
      best = current;
    }
  }
  return best;
...
const enrichRows = async (rows, { onProgress, label='embed' } = {}) => {
  const vectors = await embedTexts(rows.map((row)=>row.text), { onProgress:(done,total)=>onProgress?.(done,total,label) });
  return rows.map((row,index)=>({ ...row, vector:vectors[index] }));
};

const train = async ({ onProgress } = {}) => {
  await ensureData();
  const trainRows = await enrichRows(await flattenSplit('train'), { onProgress, label:'identifiability train embedding' });
  gate = trainLogistic(trainRows, trainRows.map((row)=>row.vector));
  const calibrationRows = await enrichRows(await flattenSplit('calibration'), { onProgress, label:'identifiability calibration embedding' });
  ...
  return { version:VERSION, threshold, trainCount:trainRows.length, calibrationCount:calibrationRows.length, calibration };
};
...
const runValidation = async ({ onProgress } = {}) => {
  if (!gate) throw new Error('请先训练并校准 Route Identifiability');
  const rows = await enrichRows(await flattenSplit('validation'), { onProgress, label:'identifiability validation embedding' });
  for (const row of rows) {
    row.probability = probabilityFromVector(row.vector);
    row.accepted = row.probability >= threshold;
    row.correct = row.accepted === row.identifiable;
  }
  ...
  return {
    ...
    identifiableByRoute:summarizeGroups(rows.filter((row)=>row.identifiable), (row)=>row.accepted),
    unresolvedByCategory:summarizeGroups(rows.filter((row)=>!row.identifiable), (row)=>!row.accepted),
    rows
  };
};

export const semanticRouteIdentifiabilityV010 = Object.freeze({
  version:VERSION,
  ...
  classify:async(text)=>{
    ...
    return { probability, identifiable:probability>=threshold, threshold };
  }
});
