...
assert(calibration.version === '0.13-scope-finalization-v0.4-calibration-v0.1', `unexpected calibration version ${calibration.version}`);
assert(calibration.status === 'presealed_fresh_scope_calibration' && calibration.sealed === false, 'Candidate v0.6 Scope calibration is not presealed');
assert(calibration.rows?.length === 222 && calibration.counts?.route_known === 132 && calibration.counts?.non_route === 90, 'Candidate v0.6 Scope calibration counts drift');
assert(calibration.policy?.parameterCount === 1 && calibration.policy?.parameterToCalibrate === 'scope_hard_veto_cutoff', 'Candidate v0.6 Scope calibration parameter contract drift');
assert(calibration.policy?.otherModelOrGateParametersMayChange === false && calibration.policy?.multiTextEncoderBatchForbidden === true, 'Candidate v0.6 Scope mutation/representation policy drift');
assert(calibration.policy?.candidateV05ScopeCalibrationExcluded === true && calibration.policy?.candidateV04ScopeCalibrationExcluded === true && calibration.policy?.allDevelopmentIndependentBlindExcluded === true && calibration.policy?.predecessorRegressionRowsExcluded === true, 'Candidate v0.6 source exclusion policy drift');
assert(patch.status === 'recorded_before_seal_and_before_scope_scoring', 'Candidate v0.6 preseal patch status drift');
assert(patch.modelOrThresholdScoredBeforePatch === false && patch.semanticRuntimeModified === false && patch.verifierWeakened === false, 'Candidate v0.6 preseal patch provenance drift');
assert(Array.isArray(patch.changes) && patch.changes.length > 0, 'Candidate v0.6 preseal patch changes missing');
...
const lock = {
  version:'0.13-scope-finalization-v0.4-calibration-lock-v0.1',
  status:'locked',
...
console.log('LiuYao Candidate v0.6 fresh Scope calibration sealed.');
