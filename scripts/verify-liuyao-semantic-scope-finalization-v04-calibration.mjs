...
assert(design.status === 'design_frozen_before_v06_calibration_data', 'Candidate v0.6 design freeze missing');
assert(calibration.version === '0.13-scope-finalization-v0.4-calibration-v0.1', `calibration version ${calibration.version}`);
...
assert(calibration.scope === 'liuyao_semantic_candidate_v0.6_scope_finalization', 'scope calibration scope drift');
assert(calibration.createdAfterCandidateV06DesignFreeze === true, 'post-v0.6-design creation marker missing');
assert(calibration.provenance?.designPath === designFile && calibration.provenance?.designSha256 === sha256(designFile), 'design provenance drift');
...
assert(calibration.policy?.fallbackAcceptanceCalibrationExcluded === true && calibration.policy?.routeabilityCalibrationExcluded === true && calibration.policy?.allDevelopmentIndependentBlindExcluded === true && calibration.policy?.predecessorRegressionRowsExcluded === true, 'scope forbidden-source policy drift');
...
for (const row of calibration.rows) {
  ...
  assert(/^SC4-\d{3}$/.test(row.id || ''), `Candidate v0.6 calibration row id drift: ${row.id}`);
...
assert(evidenceApi?.extract && arbitrationApi?.arbitrate, 'failed to load Candidate v0.6 evidence/arbitration');
...
assert(pathMismatches.length === 0, `scope calibration known path mismatches (${pathMismatches.length}): ${pathMismatches.slice(0,30).map((row)=>`${row.id}/${row.expectedRoute}/${row.expectedPath}:${row.text} actual=${row.actual} arb=${JSON.stringify(row.arbitration)} unsupported=${JSON.stringify(row.unsupported)}`).join(' | ')}`);

// The current calibration, lock, and optional pre-seal patch are the same fresh corpus, not prior evidence.
...
assert(overlaps.length === 0, `fresh v0.6 scope calibration exact overlap (${overlaps.length}): ${overlaps.slice(0,40).map((item)=>`${item.row.id}:${item.row.text}->${item.source}`).join(' | ')}`);

if (calibration.sealed) {
  assert(fs.existsSync(path.join(root, lockFile)), 'sealed v0.6 scope calibration lock missing');
  const lock = readJson(lockFile);
  assert(lock.version === '0.13-scope-finalization-v0.4-calibration-lock-v0.1' && lock.status === 'locked', 'v0.6 scope calibration lock contract drift');
  assert(lock.calibrationSha256 === sha256(calibrationFile), 'v0.6 scope calibration SHA drift');
  assert(lock.designSha256 === sha256(designFile), 'Candidate v0.6 design SHA drift');
  assert(lock.designFreezeCommit === designCommit, 'Candidate v0.6 design commit lock drift');
}

console.log('LiuYao Candidate v0.6 fresh Scope calibration corpus verified.');
console.log('- 222 total: 132 known (44 strong / 44 support / 44 pure fallback) + 90 non-route');
console.log('- all known rows match Evidence v0.5 / Arbitration v0.14 declared path contracts');
