...
assert(new Set(texts).size === 300, 'blind question text is not internally unique');
...
// Process-seal freshness: no exact question reuse from any earlier LiuYao JSON corpus.
const prior = new Map();
...
const walk = (value, source, key='') => {
  if (typeof value === 'string') {
    // Restrict to likely natural-language corpus fields; metadata strings should not define freshness.
...
for (const name of fs.readdirSync(dataDir).filter((n) => n.endsWith('.json') && n.startsWith('liuyao-') && n !== path.basename(blindPath))) {
