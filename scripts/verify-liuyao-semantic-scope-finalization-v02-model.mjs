...
const artifactFile = 'data/liuyao-semantic-scope-finalization-v0.2.json';
const lockFile = 'data/liuyao-semantic-scope-finalization-v0.2.lock.json';
...
assert(artifact.version === '0.13-scope-finalization-v0.2' && artifact.status === 'frozen_fresh_calibrated', 'Scope Finalization v0.2 artifact contract mismatch');
assert(artifact.scope === 'liuyao_semantic_candidate_v0.4_scope_finalization', 'Scope Finalization scope drift');
assert(artifact.architecture?.weightMutation === false && artifact.architecture?.otherThresholdsRetuned === false, 'Scope Finalization mutated frozen model/gate parameters');
assert(artifact.architecture?.parameter === 'one_global_hard_veto_cutoff', 'Scope Finalization parameter contract drift');
assert(artifact.architecture?.hardVetoCondition === 'scope_probability < hard_veto_cutoff', 'Scope hard-veto comparison drift');
assert(artifact.encoder?.textsPerEncoderCall === 1 && artifact.encoder?.vectorSize === 512, 'Scope Finalization representation drift');
...
assert(design.status === 'assembly_frozen_before_scope_revalidation_and_development', 'Candidate v0.4 design status drift');
...
const expectedDeps = {
  candidateV04Design:[designFile, sha256(designFile)],
...
assert(lock.version === '0.13-scope-finalization-v0.2-lock-v0.1' && lock.status === 'locked_fresh_calibrated', 'Scope Finalization lock contract mismatch');
assert(lock.artifactPath === artifactFile && lock.artifactSha256 === sha256(artifactFile), 'Scope Finalization artifact lock SHA drift');
assert(lock.calibrationSha256 === sha256(calibrationFile) && lock.calibrationLockSha256 === sha256(calibrationLockFile), 'Scope calibration provenance lock drift');
assert(lock.candidateV04DesignSha256 === sha256(designFile), 'Candidate v0.4 design lock drift');
assert(close(lock.hardVetoCutoff, artifact.hardVetoCutoff), 'Scope cutoff lock drift');
assert(lock.weightMutation === false && lock.otherThresholdsRetuned === false, 'Scope lock mutation policy drift');
...
assert(rows.length === 222, `Scope scored rows ${rows.length} != 222`);
assert(artifact.calibration?.fresh === true && artifact.calibration?.independentGeneralizationClaim === false, 'Scope calibration evidence status drift');
assert(artifact.calibration?.knownRows === 132 && artifact.calibration?.nonRouteRows === 90, 'Scope calibration count drift');
...
const evaluate = (cutoff) => {
  ...
  for (const row of known) {
    ...
    else if (final.reasonCode === 'scope_hard_veto') { scopeVeto.total += 1; if (row.preScopeRoute === row.expectedRoute) scopeVeto.correctKnown += 1; else scopeVeto.wrongKnown += 1; }
  }
  for (const subtype of subtypes) {
    const subset = nonRoute.filter((row) => row.subtype === subtype);
    let activated = 0, vetoed = 0;
    for (const row of subset) {
      const final = finalizeRow(row, cutoff);
      finalDispositions[final.disposition] = (finalDispositions[final.disposition] || 0) + 1;
      if (final.rawHardVeto) scopeVeto.rawHardVeto += 1;
...
assert(artifact.calibration.thresholdCandidateCount === candidates.length && artifact.calibration.safeThresholdCount === safeCount, 'Scope threshold search count drift');
...
for (const key of ['hardVetoCutoff','knownExact','knownTotal','knownExactRetention','knownActivated','wrongKnownActivated','acceptedRouteAccuracy','falseActivations','nonRouteTotal','overallFalseActivation','maxSubtypeFalseActivation','strongScopeBypassCount']) {
  if (typeof best[key] === 'number') assert(close(Number(stored[key]), Number(best[key])), `stored Scope metric drift: ${key}`);
}
for (const pathName of knownPaths) {
  for (const key of ['total','exact','exactRetention','activated','wrongActivated']) assert(close(Number(stored.byPath[pathName][key]), Number(best.byPath[pathName][key])), `stored path metric drift: ${pathName}.${key}`);
}
for (const subtype of subtypes) {
  for (const key of ['total','activated','falseActivation','scopeVetoed']) assert(close(Number(stored.bySubtype[subtype][key]), Number(best.bySubtype[subtype][key])), `stored subtype metric drift: ${subtype}.${key}`);
}
assert(safe(best), 'frozen Scope cutoff violates safety constraints');
...
console.log('LiuYao Candidate v0.4 Scope Finalization v0.2 calibrated artifact verified.');
