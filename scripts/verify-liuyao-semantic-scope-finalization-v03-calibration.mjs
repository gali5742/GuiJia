...
assert(design.status === 'design_frozen_before_v05_calibration_data', 'Candidate v0.5 design freeze missing');
assert(calibration.version === '0.13-scope-finalization-v0.3-calibration-v0.1', `calibration version ${calibration.version}`);
...
assert(calibration.scope === 'liuyao_semantic_candidate_v0.5_scope_finalization', 'scope calibration scope drift');
assert(calibration.createdAfterCandidateV05DesignFreeze === true, 'post-v0.5-design creation marker missing');
...
assert(evidenceApi?.extract && arbitrationApi?.arbitrate, 'failed to load Candidate v0.5 evidence/arbitration');
...
// The calibration artifact, lock, and optional pre-seal patch are the same fresh corpus, not prior evidence.
...
assert(overlaps.length === 0, `fresh v0.5 scope calibration exact overlap (${overlaps.length}): ${overlaps.slice(0,30).map((item)=>`${item.row.id}:${item.row.text}->${item.source}`).join(' | ')}`);

if (calibration.sealed) {
  assert(fs.existsSync(path.join(root, lockFile)), 'sealed v0.5 scope calibration lock missing');
  const lock = readJson(lockFile);
  assert(lock.version === '0.13-scope-finalization-v0.3-calibration-lock-v0.1' && lock.status === 'locked', 'v0.5 scope calibration lock contract drift');
  assert(lock.calibrationSha256 === sha256(calibrationFile), 'v0.5 scope calibration SHA drift');
  assert(lock.designSha256 === sha256(designFile), 'Candidate v0.5 design SHA drift');
  assert(lock.designFreezeCommit === designCommit, 'Candidate v0.5 design commit lock drift');
}

console.log('LiuYao Candidate v0.5 fresh Scope calibration corpus verified.');
console.log('- 222 total: 132 known (44 strong / 44 support / 44 pure fallback) + 90 non-route');
console.log('- all known rows match Evidence v0.4 / Arbitration v0.13 declared path contracts');
