...
const report=json(P.report), developmentLock=json(P.developmentLock), runtimeLock=json(P.runtimeLock), design=json(P.design);
...
for(const id of pathIds){
  const rows=known.filter(x=>x.expectedCandidatePath===id), exact=rows.filter(x=>x.final?.disposition==='route_known'&&x.final?.routeId===x.expectedRoute), accepted=rows.filter(x=>x.final?.disposition==='route_known'), reached=rows.filter(x=>x.reachesFallbackIdentity===true);
  byKnownPath[id]={
    n:rows.length,exact:exact.length,exactRate:rate(exact.length,rows.length),accepted:accepted.length,wrongAccepted:accepted.filter(x=>x.final?.routeId!==x.expectedRoute).length,acceptedRouteAccuracy:rate(exact.length,accepted.length),
...
for(const id of subtypeIds){
  const rows=nonRoute.filter(x=>x.nonRouteSubtype===id), activated=rows.filter(x=>x.final?.disposition==='route_known');
  bySubtype[id]={n:rows.length,falseActivations:activated.length,falseActivationRate:rate(activated.length,rows.length),semanticActRejected:rows.filter(x=>x.semanticAct?.status!=='eligible').length,routeabilityRejectedAfterEligible:rows.filter(x=>x.semanticAct?.status==='eligible'&&(!x.routeability||x.routeability.disposition!=='route_known')).length,falseActivationSources:countBy(activated,activationSource),activatedRoutes:countBy(activated,x=>x.final?.routeId||'null')};
}
const missesByRoute={};
for(const x of knownMiss){ const e=missesByRoute[x.expectedRoute]||={misses:0,causes:{}}; e.misses++; const c=failureCause(x); e.causes[c]=(e.causes[c]||0)+1; }
...
const compactKnown=x=>({id:x.id,text:x.text,expectedRoute:x.expectedRoute,expectedCandidatePath:x.expectedCandidatePath||null,failureCause:failureCause(x),semanticAct:x.semanticAct?{status:x.semanticAct.status,probability:x.semanticAct.probability,threshold:x.semanticAct.threshold}:null,arbitration:x.arbitration?{routeId:x.arbitration.routeId||null,strength:x.arbitration.strength||null}:null,routeability:x.routeability?{disposition:x.routeability.disposition,probability:x.routeability.probability,reasonCode:x.routeability.reasonCode}:null,fallbackIdentity:x.fallbackIdentity?{status:x.fallbackIdentity.decision?.status||null,routeId:x.fallbackIdentity.decision?.routeId||null,reasonCode:x.fallbackIdentity.decision?.reasonCode||null}:null,selection:x.selection?{status:x.selection.status,routeId:x.selection.routeId||null,reasonCode:x.selection.reasonCode||null}:null,final:{disposition:x.final?.disposition||null,routeId:x.final?.routeId||null,reasonCode:x.final?.reasonCode||null}});
const compactFalse=x=>({id:x.id,text:x.text,nonRouteSubtype:x.nonRouteSubtype,source:activationSource(x),semanticAct:x.semanticAct?{status:x.semanticAct.status,probability:x.semanticAct.probability,threshold:x.semanticAct.threshold}:null,arbitration:x.arbitration?{routeId:x.arbitration.routeId||null,strength:x.arbitration.strength||null}:null,routeability:x.routeability?{disposition:x.routeability.disposition,probability:x.routeability.probability,reasonCode:x.routeability.reasonCode}:null,fallbackIdentity:x.fallbackIdentity?{status:x.fallbackIdentity.decision?.status||null,routeId:x.fallbackIdentity.decision?.routeId||null,reasonCode:x.fallbackIdentity.decision?.reasonCode||null}:null,selection:x.selection?{status:x.selection.status,routeId:x.selection.routeId||null,reasonCode:x.selection.reasonCode||null}:null,final:{disposition:x.final?.disposition||null,routeId:x.final?.routeId||null,reasonCode:x.final?.reasonCode||null}});

const gates=design.freshDevelopmentPolicy.promotionGates;
const headline={knownExactRoute:rate(exactKnown.length,known.length),acceptedRouteAccuracy:rate(exactKnown.length,acceptedKnown.length),overallFalseRouteActivation:rate(falseActs.length,nonRoute.length),maxFalseRouteActivationPerSubtype:Math.max(...Object.values(bySubtype).map(x=>x.falseActivationRate)),noStructuralPathCollapse:Object.values(byKnownPath).every(x=>x.exact>0),checks:report.checks,readyForCandidateLock:false};
const dominant=Object.entries(bySubtype).sort((a,b)=>b[1].falseActivations-a[1].falseActivations)[0];
const output={
  version:'0.13-candidate-v0.4-development-failure-diagnostic-v0.1',status:'immutable_failure_diagnosis_from_committed_first_development_report',scope:'liuyao_semantic_decision_stack_v0.13_candidate_v0.4',
  provenance:{sourceReport:{path:P.report,gitBlobSha:EXPECTED.reportBlob,sha256:sha256(P.report),commit:EXPECTED.reportCommit},sealedDevelopment:{path:P.developmentLock,datasetSha256:EXPECTED.datasetSha256,sealCommit:EXPECTED.sealCommit},runtimeLock:{path:P.runtimeLock,gitBlobSha:blobSha(P.runtimeLock)},design:{path:P.design,gitBlobSha:blobSha(P.design)}},
...
console.log(JSON.stringify({sourceReportSummary:output.sourceReportSummary,headline,totals:output.totals,knownFailureCauses:output.knownFailureCauses,byKnownPath,byNonRouteSubtype:bySubtype,wrongAcceptedKnown:output.wrongAcceptedKnown.map(x=>({id:x.id,expectedRoute:x.expectedRoute,finalRoute:x.final.routeId}))},null,2));
