...
const contractPath='data/liuyao-semantic-v013-candidate-v04-fallback-identity-v02-calibration-v04-reachability-audit-contract-v0.1.json';
const calibrationLockPath='data/liuyao-semantic-v013-candidate-v04-fallback-identity-calibration-v0.4.lock.json';
const reportPath='data/liuyao-semantic-v013-candidate-v04-fallback-identity-calibration-v0.4-reachability-audit-v0.1.json';
...
assert(contract.status==='locked_before_first_postseal_calibration_v04_encoder_audit','calibration v0.4 reachability contract not frozen');
assert(calibrationLock.status==='locked','calibration v0.4 lock missing');
assert(calibrationLock.calibrationRows===704&&calibrationLock.routeKnownRows===220&&calibrationLock.nearDomainRows===440,'calibration v0.4 lock row counts drift');
...
assert(calibration.status==='sealed_fallback_stage_calibration'&&calibration.sealed===true,'calibration v0.4 not sealed');
assert(calibration.policy?.encoderScoringObserved===false,'calibration records encoder scoring before seal');
...
assert(calibrationLock.fallbackIdentityProbabilityUsed===false,'Fallback probability already used');
...
assert(schema.postSealReachabilityAudit?.requireEveryRouteHasAtLeastOneKnownReachingFallback===contract.frozenChecks.requireEveryRouteHasAtLeastOneCalibrationKnownReachingFallback,'route exposure gate drift');
...
assert(calibration.sampling?.routeKnown===220&&calibration.sampling?.nearDomain===440&&calibration.sampling?.outsideCurrent22===22&&calibration.sampling?.routeUnresolved===22,'calibration sampling drift');
...
const embedOne=async(text,index,total)=>{
  ...
  assert(normalized,'empty calibration v0.4 reachability text');
  const output=await encoder([normalized],{pooling:contract.encoder.pooling,normalize:contract.encoder.normalize});
  encoderCalls+=1;
  if((index+1)%50===0||index===total-1)console.log(`single-text calibration-v0.4 reachability embedded ${index+1}/${total}`);
...
for(let index=0;index<calibration.rows.length;index+=1){
  ...
  results.push({
    ...
    subtype:row.subtype,
    confusableFamily:row.confusableFamily??null,
...
for(const routeId of [...new Set(known.map((row)=>row.expectedRoute))].sort()){
  ...
  byRoute[routeId]={n:subset.length,semanticActEligible:subset.filter((row)=>row.semanticAct.eligible).length,arbitrationNull:subset.filter((row)=>!row.arbitration).length,routeabilityAccepted:subset.filter((row)=>row.routeability.accepted).length,reachesFallback:subset.filter((row)=>row.reachesFallback).length};
}
...
for(const subtype of [...new Set(nonRoute.map((row)=>row.subtype))].sort()){
  ...
  byNonRouteSubtype[subtype]={n:subset.length,semanticActEligible:subset.filter((row)=>row.semanticAct.eligible).length,arbitrationNull:subset.filter((row)=>!row.arbitration).length,routeabilityAccepted:subset.filter((row)=>row.routeability.accepted).length,reachesFallback:subset.filter((row)=>row.reachesFallback).length};
}
...
const report={
  version:'0.13-candidate-v0.4-fallback-identity-v0.2-calibration-v0.4-reachability-audit-v0.1',
  status:pass?'pass_locked_sealed_calibration_v04_reachability':'fail_locked_sealed_calibration_v04_reachability',
  ...
  policy:{auditOnly:true,fallbackIdentityWeightsTrained:false,fallbackIdentityProbabilitiesScored:false,fallbackThresholdSelected:false,routerLoaded:false,routerTopKRead:false,scopeHardVetoUsedForReachability:false,v03ReachabilityRowResultsRead:false,independentEvaluationRead:false,sealedBlindEvaluationRead:false,candidateV03FailureRowsRead:false,newThemeResearchRead:false,corpusMutationAllowed:false},
  ...
  summary:{calibrationKnown:known.length,calibrationNonRoute:nonRoute.length,calibrationKnownSemanticActRetention:knownSemanticActRetention,calibrationKnownReachingFallback:knownReachingFallback,calibrationNonRouteReachingFallback:nonRouteReachingFallback,nearDomainReachingFallback},
  byRoute,
  byNonRouteSubtype,
  checks,
  pass,
  nextAction:pass?'freeze_fallback_identity_v02_training_and_calibration_contract_before_any_identity_training_or_probability_scoring':'freeze_this_calibration_v04_failure_and_create_new_versioned_stage_specific_calibration_without_mutating_current_sealed_data',
  results
};
writeJson(reportPath,report);
console.log('Candidate v0.4 Fallback Identity v0.2 calibration v0.4 post-seal reachability audit complete.');
