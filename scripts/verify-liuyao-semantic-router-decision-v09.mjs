...
const wordingPatch = JSON.parse(read('data/liuyao-semantic-router-decision-v0.9-development-patch.json'));
...
assert(runtime.includes("liuyao-semantic-router-decision-v0.9-development-patch.json"), 'v0.9 runtime must apply the verified pre-run wording patch');
...
assert(wordingPatch.version === '0.9-dev-wording-patch-0.1' && wordingPatch.status === 'development_preuse_patch', 'v0.9 wording patch contract mismatch');
assert(wordingPatch.base === 'liuyao-semantic-router-decision-v0.9-development.json', 'v0.9 wording patch base drifted');
...
console.log('- verified pre-run wording isolation patch is part of runtime contract');
