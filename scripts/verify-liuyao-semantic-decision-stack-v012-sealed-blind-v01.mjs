...
const replacements=patch.replacements||{};assert(Object.keys(replacements).length===1,'expected exactly 1 v0.12 pre-use wording correction');for(const id of Object.keys(replacements))assert(rows.some(r=>r.id===id),`patch references unknown Blind id ${id}`);for(const row of rows)if(replacements[row.id])row.text=replacements[row.id];
...
console.log('- 1 pre-use wording-only seal correction applied');
