...
const patchName='liuyao-semantic-decision-stack-v0.12-sealed-blind-v0.1-preuse-patch.json';
const read=p=>JSON.parse(fs.readFileSync(path.join(root,p),'utf8'));
const data=read(`data/${targetName}`);const patch=read(`data/${patchName}`);const inventory=read('data/liuyao-semantic-route-inventory-v0.2.json');
...
assert(patch.version==='0.1-preuse-wording-patch'&&patch.status==='sealed_preuse_patch'&&patch.base===targetName,'v0.12 Blind pre-use patch mismatch');
...
const replacements=patch.replacements||{};assert(Object.keys(replacements).length===10,'expected exactly 10 v0.12 pre-use wording corrections');for(const id of Object.keys(replacements))assert(rows.some(r=>r.id===id),`patch references unknown Blind id ${id}`);for(const row of rows)if(replacements[row.id])row.text=replacements[row.id];
...
const isPostV012StackCorpus=name=>{const m=name.match(/^liuyao-semantic-decision-stack-v0\.(\d+)/);return m?Number(m[1])>=13:false;};
for(const name of fs.readdirSync(dataDir).filter(n=>n.startsWith('liuyao-')&&n.endsWith('.json')&&n!==targetName&&n!==patchName&&!isPostV012StackCorpus(n))){walk(JSON.parse(fs.readFileSync(path.join(dataDir,name),'utf8')),name);}
...
console.log('- 10 pre-use wording-only seal corrections applied');
console.log('- overlap audit is frozen against corpora available through the v0.12 seal');
