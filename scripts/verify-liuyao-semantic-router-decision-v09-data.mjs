...
const remember = (text, bucket) => {
  assert(typeof text === 'string' && text.trim(), `empty text in ${bucket}`);
  const key = normalize(text);
  assert(key, `empty normalized text in ${bucket}`);
  assert(!seen.has(key), `duplicate v0.9 development text: ${text} in ${bucket} and ${seen.get(key)}`);
  seen.set(key, bucket);
};
...
assert(seen.size === 352, `v0.9 development unique count ${seen.size} != 352`);
