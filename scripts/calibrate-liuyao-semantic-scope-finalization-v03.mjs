...
const outputFile = 'data/liuyao-semantic-scope-finalization-v0.3.json';
const outputLockFile = 'data/liuyao-semantic-scope-finalization-v0.3.lock.json';
...
assert(calibration.status === 'sealed_fresh_scope_calibration' && calibration.sealed === true, 'Candidate v0.5 Scope calibration must be sealed before scoring');
assert(calibrationLock.status === 'locked' && calibrationLock.calibrationSha256 === sha256(calibrationFile), 'Candidate v0.5 Scope calibration lock drift');
assert(calibrationLock.designSha256 === sha256(designFile), 'Candidate v0.5 design SHA drift');
assert(calibrationLock.policy?.parameterCount === 1 && calibrationLock.policy?.parameter === 'scope_hard_veto_cutoff', 'Candidate v0.5 Scope calibration parameter contract drift');
assert(calibrationLock.policy?.otherModelOrGateParametersMayChange === false && calibrationLock.policy?.multiTextEncoderBatchForbidden === true, 'Candidate v0.5 Scope calibration mutation/representation policy drift');
...
assert(calibration.rows?.length === 222, 'Candidate v0.5 Scope calibration count drift');
...
assert(evidenceApi?.extract && arbitrationApi?.arbitrate && compatibilityApi?.evaluate, 'Candidate v0.5 semantic evidence modules unavailable');
assert(routeabilityApi?.decide && selectionApi?.decide && finalizationApi?.finalize, 'Candidate v0.5 runtime modules unavailable');
...
for (let index = 0; index < calibration.rows.length; index += 1) {
  ...
  if ((index + 1) % 50 === 0 || index + 1 === calibration.rows.length) console.log(`scope finalization v0.3 embedded ${index + 1}/${calibration.rows.length}`);
}
...
assert(Number.isFinite(constraints.minimumAcceptedRouteAccuracy) && Number.isFinite(constraints.maximumOverallFalseRouteActivation) && Number.isFinite(constraints.maximumFalseRouteActivationPerNonRouteSubtype), 'Candidate v0.5 safety constraints missing');
...
const artifact = {
  version:'0.13-scope-finalization-v0.3',
  ...
  dependencies:{
    candidateV05Design:{ path:designFile, sha256:sha256(designFile) },
  ...
  calibration:{
    ...
    objective:'maximize_final_known_exact_retention_subject_to_candidate_v0.5_safety_constraints',
  ...
  nextEvidenceRequirement:'fresh_prelock_candidate_v0.5_full_stack_development_then_candidate_lock_then_fresh_post_lock_independent'
};
writeJson(outputFile, artifact);
const lock = {
  version:'0.13-scope-finalization-v0.3-lock-v0.1',
  ...
  candidateV05DesignSha256:sha256(designFile),
  hardVetoCutoff:best.hardVetoCutoff,
  safety:{
...
console.log('LiuYao Candidate v0.5 Scope Finalization v0.3 fresh calibration complete.');
