...
const routeabilityArtifact = readJson('data/liuyao-semantic-routeability-v0.3.json');
const rows = baseline.results || [];
if (rows.length !== 198) throw new Error(`baseline result rows ${rows.length} != 198`);
if (routeabilityArtifact.status !== 'frozen' || !Number.isFinite(routeabilityArtifact.calibration?.threshold)) throw new Error('frozen Routeability v0.3 missing');
const threshold = routeabilityArtifact.calibration.threshold;
...
if (!evidenceExtractor?.extract || !arbitration?.arbitrate || !selection?.decide || !routeability?.decide) throw new Error('refined v0.13 modules failed to load');
...
const replayed = rows.map((row) => {
  ...
  const gate = routeability.decide({ probability:row.routeability.probability, threshold, arbitration:arb, evidence });
  let selected = null;
  ...
  return {
    ...
    routeability:{ probability:row.routeability.probability, threshold, disposition:gate.disposition, reasonCode:gate.reasonCode },
    head:row.head,
    scope:row.scope,
    arbitration:arb,
...
const report = {
  version:'0.13-development-report-v0.3',
  status:'tuning_diagnostic_after_v03_freeze',
  scope:'liuyao_semantic_decision_stack_v0.13',
  warning:'This 198-row set informed earlier semantic design. These metrics are tuning diagnostics only and must not be cited as independent generalization evidence.',
  candidate:{ evidence:'v0.2', arbitration:'v0.12', compatibility:'v0.2', selection:'v0.2', routeability:'frozen_v0.3', routeabilityArtifactSha256:readJson('data/liuyao-semantic-routeability-v0.3.lock.json').artifactSha256 },
  summary,
  failures:replayed.filter((row) => !row.finalExact),
  results:replayed
};
fs.writeFileSync(path.join(root, 'data/liuyao-semantic-decision-stack-v0.13-development-report-v0.3.json'), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
