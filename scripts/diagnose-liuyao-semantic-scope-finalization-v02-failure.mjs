...
const reportFile = 'data/liuyao-semantic-scope-finalization-v0.2-calibration-failure.json';
...
assert(design.status === 'assembly_frozen_before_scope_revalidation_and_development', 'Candidate v0.4 design drift');
assert(calibration.status === 'sealed_fresh_scope_calibration' && calibration.sealed === true, 'sealed Scope calibration required');
assert(calibrationLock.calibrationSha256 === sha256(calibrationFile), 'Scope calibration lock drift');
...
for(let i=0;i<calibration.rows.length;i+=1){
  ...
  rows.push({id:source.id,expectedDisposition:source.expectedDisposition,expectedRoute:source.expectedRoute,expectedCandidatePath:source.expectedCandidatePath,subtype:source.subtype,routeabilityDisposition:routeabilityDecision.disposition,arbitrationRoute:arbitration?.routeId||null,arbitrationStrength:arbitration?.strength||null,fallbackStatus:fallback?.status||null,fallbackRoute:fallback?.routeId||null,selectionStatus:selection?.status||null,selectionRoute:selection?.routeId||null,scopeProbability:sp,strongScopeBypassEligible:bypass,preScopeDisposition:pre.disposition,preScopeRoute:pre.routeId,preScopeReasonCode:pre.reasonCode});
  if((i+1)%50===0||i+1===calibration.rows.length)console.log(`scope failure diagnostic embedded ${i+1}/${calibration.rows.length}`);
}
...
const sortCopy=(fn)=>[...metrics].sort(fn)[0];
const bestKnown=sortCopy((a,b)=>b.knownExact-a.knownExact||b.acceptedRouteAccuracy-a.acceptedRouteAccuracy||a.overallFalseActivation-b.overallFalseActivation||a.maxSubtypeFalseActivation-b.maxSubtypeFalseActivation||b.cutoff-a.cutoff);
const minOverall=sortCopy((a,b)=>a.overallFalseActivation-b.overallFalseActivation||a.maxSubtypeFalseActivation-b.maxSubtypeFalseActivation||b.knownExact-a.knownExact||b.cutoff-a.cutoff);
const minSubtype=sortCopy((a,b)=>a.maxSubtypeFalseActivation-b.maxSubtypeFalseActivation||a.overallFalseActivation-b.overallFalseActivation||b.knownExact-a.knownExact||b.cutoff-a.cutoff);
const maxAccuracy=sortCopy((a,b)=>b.acceptedRouteAccuracy-a.acceptedRouteAccuracy||b.knownExact-a.knownExact||a.overallFalseActivation-b.overallFalseActivation||b.cutoff-a.cutoff);
const atZero=evaluate(0), atOne=evaluate(1);
const irreducibleNonRoute=nonRoute.filter((row)=>finalize(row,1).disposition==='route_known').map((row)=>({id:row.id,subtype:row.subtype,selectedRoute:row.preScopeRoute,arbitrationRoute:row.arbitrationRoute,arbitrationStrength:row.arbitrationStrength,strongScopeBypassEligible:row.strongScopeBypassEligible,scopeProbability:row.scopeProbability}));
const irreducibleWrongKnown=known.filter((row)=>{const f=finalize(row,1);return f.disposition==='route_known'&&f.routeId!==row.expectedRoute;}).map((row)=>({id:row.id,expectedRoute:row.expectedRoute,selectedRoute:row.preScopeRoute,path:row.expectedCandidatePath,arbitrationRoute:row.arbitrationRoute,strongScopeBypassEligible:row.strongScopeBypassEligible,scopeProbability:row.scopeProbability}));
const report={
  version:'0.13-scope-finalization-v0.2-calibration-failure-v0.1',status:'calibration_failed_no_safe_cutoff',scope:'liuyao_semantic_candidate_v0.4_scope_finalization',candidateV04Lockable:false,
  ...
  conclusion:'No global Scope hard-veto cutoff satisfies the frozen Candidate v0.4 safety constraints. Candidate v0.4 must not be locked or promoted from this calibration.'
};
writeJson(reportFile,report);
console.log('LiuYao Candidate v0.4 Scope cutoff failure diagnosed.');
console.log(`- safe thresholds: ${allSafe.length}/${thresholds.length}`);
console.log(`- feasibility: accuracy=${report.feasibility.accuracyOnly}, overallFA=${report.feasibility.overallFalseActivationOnly}, subtypeFA=${report.feasibility.subtypeFalseActivationOnly}, accuracy+overall=${report.feasibility.accuracyAndOverall}, accuracy+subtype=${report.feasibility.accuracyAndSubtype}, overall+subtype=${report.feasibility.overallAndSubtype}`);
...
console.log(`- irreducible wrong-known activations at cutoff=1: ${irreducibleWrongKnown.length}`);
